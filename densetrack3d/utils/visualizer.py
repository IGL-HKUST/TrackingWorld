...
def flow_to_rgb(flow, transparent=False):
    ...
    magnitude = np.clip(norm * scaling, 0.0, 1.0)
    ...
    def draw_tracks_on_video(
        ...
        if self.mode == "optical_flow":
            ...
            if self.mode == "rainbow":
                ...
                for n in range(N):
                    color = self.color_map(norm(tracks[-1, n, 1]))
        ...
        if default_color is not None:
            ...
            vector_colors = np.repeat(vector_colors[:, None], N, axis=1)
        ...
        if self.tracks_leave_trace != 0:
            for t in range(query_frame + 1, T):
                ...
                curr_vis = visibility[0, t].cpu().numpy()
                if compensate_for_camera_motion:
                ...
                res_video[t] = self._draw_pred_tracks(
                    res_video[t],
                    curr_tracks[:, curr_vis[:, 0]],
                    curr_colors[:, curr_vis[:, 0]],
                )
                if gt_tracks is not None:
