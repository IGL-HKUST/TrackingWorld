    ...
    def initialize_mlp(self):
        for layer in self.mlp:
            if isinstance(layer, nn.Linear):
                if layer.weight.shape[0] == layer.weight.shape[1]:  # 方阵时
                    nn.init.eye_(layer.weight)  # 单位矩阵
                else:
                    # 只对 lower_fmaps 部分初始化为单位矩阵，其余为 0
                    nn.init.zeros_(layer.weight)
                    nn.init.eye_(layer.weight[:, :self.consist_dim])
